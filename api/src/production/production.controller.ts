// api/src/production/production.controller.ts
...
export class ProductionController {
  ...
  @Get('recipes/:orgId')
  getRecipes(@Param('orgId') orgId: string) { 
    return this.productionService.getRecipes(orgId); 
  }
  ...
  getOrders(@Param('branchId') branchId: string) { 
    return this.productionService.getOrders(branchId); 
  }
  ...
  updateOrderStatus(
    ...
    @Request() req: any // Extracts user data from JWT token
  ) { 
    return this.productionService.updateOrderStatus(id, dto, req.user.userId); 
  }
}
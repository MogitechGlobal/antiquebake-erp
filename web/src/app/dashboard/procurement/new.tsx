...
interface PurchaseOrder {
  id: string;
  poNumber: string;
  status: 'DRAFT' | 'APPROVED' | 'RECEIVED' | 'CANCELLED'; // Updated to match PHP workflow
  totalAmount: number;
  createdAt: string;
  supplier: Supplier;
  items: POItem[];
...
export default function ProcurementDashboardPage() {
  ...
  // Role-Based Control Check (Matching PHP Logic)
  // In PHP: in_array($userRole, ['StoreManager', 'Admin', 'Super Admin'])
  ...
  const handleCreateOrder = async (e: React.FormEvent, isDirectGrn: boolean) => {
    ...
    try {
      // Create the base order. Initial status depends on role (Manager = APPROVED, normal = DRAFT)
      const initialStatus = isManager ? 'APPROVED' : 'DRAFT';
      
      const res = await axios.post(`${API_URL}/api/v1/procurement/order`, { 
        supplierId: lpoForm.supplierId, 
        branchId: user.branchId,
        items: validItems,
        status: isDirectGrn ? 'RECEIVED' : initialStatus // Override if direct GRN
      }, axiosConfig);
  ...
  // Status Update Handler (Handles Approve, Cancel, Reset, Receive)
  const handleOrderStatusUpdate = async (orderId: string, newStatus: string) => {
    ...
    if (newStatus === 'RECEIVED') confirmMsg = "Confirming receipt will update your inventory levels automatically. Proceed?";
            ...
            <div className="p-4 border-t border-zinc-200 bg-zinc-100 flex justify-between items-center print:hidden">
              <div className="flex space-x-2">
                 ...
                 {isManager && selectedOrder.status === 'DRAFT' && (
                    <button onClick={() => handleOrderStatusUpdate(selectedOrder.id, 'APPROVED')} disabled={isSubmitting} className="px-4 py-2 text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors">
                       <CheckCircle2 className="w-3.5 h-3.5 inline mr-1" /> Approve
                    </button>
                 )}
                 {isManager && (selectedOrder.status === 'DRAFT' || selectedOrder.status === 'APPROVED') && (
                    <button onClick={() => handleOrderStatusUpdate(selectedOrder.id, 'CANCELLED')} disabled={isSubmitting} className="px-4 py-2 text-xs font-bold text-red-600 bg-red-50 border border-red-200 hover:bg-red-100 rounded-lg transition-colors">
                       <X className="w-3.5 h-3.5 inline mr-1" /> Cancel Order
                    </button>
                 )}
                 {isManager && selectedOrder.status === 'CANCELLED' && (
                    <button onClick={() => handleOrderStatusUpdate(selectedOrder.id, 'DRAFT')} disabled={isSubmitting} className="px-4 py-2 text-xs font-bold text-zinc-700 bg-white border border-zinc-300 hover:bg-zinc-50 rounded-lg transition-colors">
                       <RotateCcw className="w-3.5 h-3.5 inline mr-1" /> Reset to Draft
                    </button>
                 )}
              </div>
              <div className="flex space-x-3">
                ...
                {isManager && selectedOrder.status === 'APPROVED' && (
                  <button onClick={() => handleOrderStatusUpdate(selectedOrder.id, 'RECEIVED')} disabled={isSubmitting} className="px-6 py-2.5 text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 shadow-lg shadow-emerald-600/20 rounded-xl flex items-center transition-all">
                    {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin mr-2"/> : <Box className="w-4 h-4 mr-2" />}
                    Receive Stock (GRN)
                  </button>
                )}
              </div>
